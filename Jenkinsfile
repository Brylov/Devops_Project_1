        ...
        stage('Run Images') {
            steps {
                ...
                script {
                    ...
                    docker.image('backend-test').run("--rm --name backend_jenkins_test -m 2g --memory-swap 2g -p 5000:5000 --network internal_tests --network ${DOCKER_NETWORK} --env-file .env --env JENKINS_TESTS=true --link mongodb_jenkins_test:mongodb")
